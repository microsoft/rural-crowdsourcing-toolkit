...
interface MicrotaskAssignmentDaoExtra {
    ...
    /**
     * Query to get all unsubmitted microtask assignments for a given [taskId].
     * [includeCompleted] specifies if completed assignments that are not yet submitted should be
     ...
     */
    @Query("SELECT ma.* FROM microtask_assignment AS ma INNER JOIN karya_file AS kf ON ma.output_file_id = kf.id WHERE kf.in_server=:in_server")
