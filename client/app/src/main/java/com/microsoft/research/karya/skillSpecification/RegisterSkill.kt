...
import com.microsoft.research.karya.database.models.WorkerLanguageSkillRecord
...
class RegisterSkill :
    NetworkActivity(
        indeterminateProgress = true, noMessage = false, allowRetry = true, needIdToken = true) {
  ...
  override suspend fun getStringsForActivity() {
    ...
    currentSkill = karyaDb.workerLanguageSkillDao().getAll().find { it.language_id == languageId }
    ...
    val skillCall =
        if (newSkill) {
          ...
          karyaAPI.registerSkill(
              skillObject, thisWorker.auth_provider.toString(), thisWorker.id_token!!)
        } else {
          val skillObject =
              WorkerLanguageSkillObject(
  ...
  override fun startNextActivity() {
    when (intent.getIntExtra(AppConstants.SKILL_SPECIFICATION_CALLER, 0)) {
      AppConstants.FETCH_FILE_FOR_APP_LANGUAGE, AppConstants.SKILLED_LANGUAGE_LIST -> {
        finish()
      }
      AppConstants.SPLASH_SCREEN, AppConstants.REGISTER_WORKER -> {
        val nextIntent = Intent(applicationContext, SkilledLanguageList::class.java)
        nextIntent.putExtra(AppConstants.SKILLED_LANGUAGE_LIST_CALLER, AppConstants.REGISTER_SKILL)
