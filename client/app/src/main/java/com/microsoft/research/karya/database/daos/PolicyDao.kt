...
import com.microsoft.research.karya.database.models.PolicyRecord

@Dao
interface PolicyDao : BasicDao<PolicyRecord> {

  @Query("SELECT * FROM policy") suspend fun getAll(): List<PolicyRecord>

  @Query("SELECT * FROM policy WHERE id == :id") suspend fun getById(id: Int): PolicyRecord
