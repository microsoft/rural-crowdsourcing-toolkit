...
interface WorkerDaoExtra {
  @Query("UPDATE worker SET app_language=:language_id") suspend fun updateAppLanguage(language_id: Int)
  ...
  @Query("UPDATE worker SET id_token=:id_token") suspend fun updateIdToken(id_token: String)

  @Query("UPDATE worker SET params=:params") suspend fun updateParams(params: JsonObject)
}
