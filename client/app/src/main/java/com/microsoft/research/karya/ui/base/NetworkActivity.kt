...
abstract class NetworkActivity(
    ...
    /**
     * Function to start the next activity. Called right after executing network request in
     * UI scope.
     */
    ...
    private fun sendNetworkRequest() {
        resetError()
        var success = true
        val networkRequestJob = ioScope.launch {
            try {
                if (needIdToken) {
                    checkIdToken()
                }
                ...
                success = false
                //TODO: Set a good error message., maybe by passing an error message inside the function?
                setErrorMessage()
    ...
    private suspend fun checkIdToken() {
        ...
        if (current - body.iat > DAY45_IN_SECONDS || current > body.exp) {
            val updatedWorkerResponse = karyaAPI.refreshIdToken(
