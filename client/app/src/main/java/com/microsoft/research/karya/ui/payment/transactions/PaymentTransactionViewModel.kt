...
@HiltViewModel
class PaymentTransactionViewModel @Inject constructor(
    ...
    fun fetchTransactions() {
        viewModelScope.launch {
            ...
            paymentRepository.getTransactions(idToken, worker.id).catch {
                _uiStateFlow.update { it.copy(isLoading = false, errorMessage = "Error fetching transactions") }
            }.collect { list ->
                val transactionList = list.map { transaction ->
                ...
                _uiStateFlow.update { it.copy(isLoading = false, errorMessage = "", userTransactionDetailList = transactionList) }
            }
