...
package com.microsoft.research.karya.scenarios.speechData
...
import com.microsoft.research.karya.scenarios.common.MicrotaskRenderer
import com.microsoft.research.karya.scenarios.speechData.SpeechDataMain.ButtonState.*
...
open class SpeechDataMain(
     ...
     * UI button states
     *
     * [DISABLED]: Greyed out. Cannot click
     * [ENABLED]: Can click
     * [ACTIVE]: Red color. Can click
     */
    ...
    final override fun setupActivity() {
        ...
        recordBtnCv.addOnLayoutChangeListener { _: View, left: Int, _: Int, right: Int, _: Int, _: Int, _: Int, _: Int, _: Int ->
    ...
    final override fun cleanupOnStop() {
        ...
        runBlocking {
            when (previousActivityState) {
                 ...
                 */
                ActivityState.SIMPLE_NEXT, ActivityState.SIMPLE_BACK -> {
                    preRecordingJob?.join()
                    releaseRecorder()
                }
     ...
     * that sentence. Create the wav file in the scratch folder and write the WAV header. Depending
     * on whether the current microtask is completed or not, move to PRERECORDING or
    ...
    private fun setActivityState(targetState: ActivityState) {
        ...
        when (activityState) {
            /**
             * INIT: release audio recorder and media player. May not be necessary? Microtask
             * setup will transition to next state
             */
            ActivityState.INIT -> {
                releasePlayer()
                releaseRecorder()
            ...
            /**
             * RECORDING: If not coming from prerecording states, initialize the audio recorder and
             * start recording. Start chronometer. Write audio data to file.
             */
            ActivityState.RECORDING -> {
            ...
            /**
             * FIRST_PLAYBACK: Start media player and play the scratch wav file
             *  If coming from paused state, resume player
             */
            ActivityState.FIRST_PLAYBACK -> {
             ...
             * player. Set the onCompletion listener to go back to the completed state. Play old
             * output file.
             */
            ActivityState.OLD_PLAYING -> {
            ...
            /**
             * SIMPLE_NEXT: If prerecording, then wait for prerecording job to finish. Then move to
             * next microtask
             */
            ActivityState.SIMPLE_NEXT -> {
                runBlocking {
            ...
            /**
             * ENCODING_BACK: Encode scratch file to compressed output file. Move to previous
             * microtask.
             */
            ActivityState.ENCODING_BACK -> {
                runBlocking {
            ...
            ActivityState.ACTIVITY_STOPPED -> {
                /**
                 * This is a dummy state to trigger events (e.g., end recordings). [cleanupOnStop]
                 * should take care of handling actual cleanup.
                 */
            }
        }
    }
    ...
    private fun playRecordAction() {
        uiScope.launch {
            playAssistantAudio(R.string.audio_record_action,
                uiCue = {
    ...
    private fun playStopAction() {
        uiScope.launch {
            playAssistantAudio(R.string.audio_stop_action,
                uiCue = {
                    recordPointerIv.visibility = View.VISIBLE
                },
                onCompletionListener = {
    ...
    private fun playListenAction() {
        playAssistantAudio(R.string.audio_listen_action,
            uiCue = {
    ...
    private fun playRerecordAction() {
        playAssistantAudio(R.string.audio_rerecord_action,
            uiCue = {
                recordPointerIv.visibility = View.VISIBLE
                recordBtn.setBackgroundResource(R.drawable.ic_mic_enabled)
            },
    ...
    private fun playNextAction() {
        playAssistantAudio(R.string.audio_next_action,
            uiCue = {
    ...
    private fun playPreviousAction() {
        playAssistantAudio(R.string.audio_previous_action,
            uiCue = {
    ...
    private fun handleRecordClick() {
        ...
        when (activityState) {
            /**
             * Prerecording states: Set target button states. Wait for wave file init job and
             * prerecording job to finish. Write the prerecord buffer to wav file. Start regular
             * recording.
             */
            ActivityState.PRERECORDING, ActivityState.COMPLETED_PRERECORDING -> {
            ...
            ActivityState.COMPLETED -> {
                ...
                scratchRecordingFileInitJob = ioScope.launch {
                    resetWavFile()
                }
            ...
            /**
             * Media player states: Stop and release media player. Reset wav file. Restart
             * recording.
             */
    ...
    private fun writeAudioDataToPrerecordBuffer() {
        ...
        preRecordingJob = ioScope.launch {
            while (isPrerecordingState(activityState)) {
                ...
                val readBytes = audioRecorder!!.read(
                    currentBuffer, consumedBytes, remainingBytes
                )
    ...
    private fun finishRecordingAndFinalizeWavFile() {
        runBlocking {
            audioFileFlushJob = ioScope.launch {
            ...
            /**
             * If still in recorded state, switch to playback. User may have stopped activity by
