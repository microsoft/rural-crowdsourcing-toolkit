...
class SelectAppLanguage :
    BaseActivity(useAssistant = true, playAssistantOnResume = false),
  ...
  private fun playPromptForLanguage(index: Int) {
    uiScope.launch {
      ...
      playAssistantAudio(
          ...
          languageId,
          uiCue = { blinkPointer(index) },
          onCompletionListener = { playNext(index) })
    }
  }
  ...
  private fun playNext(index: Int) {
    uiScope.launch {
      ...
      if (index != supportedLanguages.lastIndex) playPromptForLanguage(index + 1)
    }
  }
  ...
  private fun blinkPointer(index: Int) {
    ...
    assistantTimer!!.scheduleAtFixedRate(0, 400) {
      uiScope.launch { setPointer(index, !supportedLanguages[index].showPointer) }
    }
  }
  ...
  private fun getSupportedLanguages() =
      ioScope.launch {
        ...
        for (l in supportedLanguageRecords) {
          ...
          supportedLanguages.add(SupportedLanguage(l.id, l.primary_language_name, prompt, false))
        }
      }
  ...
  override fun onLanguageSelected(language: SupportedLanguage) {
    ...
    if (fromActivity == AppConstants.DASHBOARD) {
      ioScope.launch { karyaDb.workerDaoExtra().updateAppLanguage(language.id) }
    }
