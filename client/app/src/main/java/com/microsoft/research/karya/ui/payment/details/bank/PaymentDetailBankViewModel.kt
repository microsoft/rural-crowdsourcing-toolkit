...
@HiltViewModel
class PaymentDetailBankViewModel @Inject constructor(
    ...
    private fun validateInputs(
        ...
        if (name.length < 3) {
            _uiStateFlow.update {
                it.copy(isLoading = false, errorMessage = "Name cannot be less than 3 characters, please enter your complete name")
            }
            return false
        }
        ...
        if (ifsc.isEmpty() || ifsc.length < 11) {
            _uiStateFlow.update {
                it.copy(
                    isLoading = false,
                    errorMessage = "ifsc should be 11 characters long, please check your ifsc code"
                )
            }
            return false
        }
        ...
        if (accountNumber.isEmpty() || accountNumberRepeated.isEmpty()) {
            _uiStateFlow.update {
                it.copy(
                    isLoading = false,
                    errorMessage = "account number cannot be empty, please enter your account number"
                )
            }
            return false
        }
