 ...
 * @param cutoutRoundedCornerRadius
 * ```
 * The radius of the each of the corners where the semicircle
 * ```
 *      meets the straight edge.
 * @param cutoutVerticalOffset
 * ```
 * The amount the cutout should be lifted up in relation to the circle's
 * ```
 *      middle.
 * @param cutoutDiameter
 * ```
 * The diameter of the semicircle to be cutout.
...
class SemiCircleEdgeCutoutTreatment(
  ...
  override fun getEdgePath(length: Float, center: Float, interpolation: Float, shapePath: ShapePath) {
    ...
    verticalOffset = interpolation * cutoutVerticalOffset + (1 - interpolation) * cradleRadius
    ...
    distanceX = sqrt((distanceBetweenCentersSquared - distanceY * distanceY).toDouble()).toFloat()
