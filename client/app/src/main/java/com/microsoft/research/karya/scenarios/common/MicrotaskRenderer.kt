...
package com.microsoft.research.karya.scenarios.common
...
import com.microsoft.research.karya.database.models.MicrotaskAssignmentRecord
import com.microsoft.research.karya.database.models.MicrotaskAssignmentStatus
import com.microsoft.research.karya.database.models.MicrotaskRecord
...
/**
 * Abstract microtask renderer class. Each scenario supported in the karya platform should
...
abstract class MicrotaskRenderer(
    ...
    /**
     * Get the unique file name of the output for current assignment. [params] is a pair of
     * strings: a file identifier and extension. The file name is usually the current
     * assignmentID appended with the identifier. The full file name is unique for a
     * unique [params] pair.
     */
    ...
    protected suspend fun completeAndSaveCurrentMicrotask() {
        ...
        val deleteScratchFilesJob = ioScope.launch {
            val directory = getDir("microtask-assignment-scratch", Context.MODE_PRIVATE)
        ...
        karyaDb.microtaskAssignmentDaoExtra().markComplete(
    ...
    /**
     * Move to previous microtask and setup. Returns false if there is no previous microtask.
     * Else true
     */
    ...
    final override fun onCreate(savedInstanceState: Bundle?) {
        ...
        runBlocking {
            /** Fetch the task */
            ioScope.launch {
                task = karyaDb.taskDao().getById(taskId)
            }.join()

            setWorkerJob.join()
            firstTimeActivityVisit = try {
        ...
        microtaskLoadingJob = ioScope.launch {
            microtaskAssignmentIDs = karyaDb.microtaskAssignmentDaoExtra().getUnsubmittedIDsForTask(
                task.id, includeCompleted
            )
    ...
    private fun getAndSetupMicrotask() {
        runBlocking {
            ...
            if (currentMicrotask.input_file_id != null) {
                ...
                if (!File(microtaskTarBallPath).exists()) {
                    ...
                    uiScope.launch {
                        ...
                        alertDialogBuilder.setMessage("Input files were not fully downloaded. Please sync with server to download all files")
            ...
            outputData = if (currentAssignment.output.has("data")) {
                currentAssignment.output.getAsJsonObject("data")
            } else {
                JsonObject()
            }

            logs = if (currentAssignment.output.has("logs")) {
                currentAssignment.output.getAsJsonArray("logs")
            } else {
                JsonArray()
            }

            outputFiles = if (currentAssignment.output.has("files")) {
