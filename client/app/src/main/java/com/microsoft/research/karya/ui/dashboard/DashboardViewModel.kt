...
constructor(
  ...
  suspend fun refreshList() {
    ...
    taskInfoList.forEach { taskInfo ->
      ...
      tempList.add(
        TaskInfo(taskInfo.taskID, taskInfo.taskName, taskInfo.scenarioName, taskStatus, taskInfo.isGradeCard)
      )
    }
          ...
          taskList.forEach { taskRecord ->
            ...
            if (taskRecord.scenario_name.equals("SIGN_LANGUAGE_VIDEO")) {
              if (taskStatus.assignedMicrotasks > 0 || taskStatus.completedMicrotasks > 0) {
                tempList.add(
                  TaskInfo(taskRecord.id, taskRecord.display_name, taskRecord.scenario_name, taskStatus, false)
                )
              }
  ...
  private suspend fun getWorkerAccountInfo(): Float {
    val worker = authManager.getLoggedInWorker()
    paymentRepository
      .getCurrentAccount(worker.idToken!!)
      .catch {
        // TODO: There was an error updating the account
      }
      .collect { paymentRepository.updatePaymentRecord(worker.id, it) }
    val workerBalanceResponse =
      paymentRepository
        .getWorkerBalance(worker.idToken, worker.id)
        .catch { emit(WorkerBalanceResponse(-1.0f, -1.0f)) }
        .single()
