...
class BoxSyncWorker(context: Context, workerParams: WorkerParameters) : CoroutineWorker(context, workerParams) {
    ...
    private suspend fun updateProgressStage(progressStage: Progress) = withContext(Dispatchers.IO) {
        _progressStage.emit(progressStage)
    }
...
//        val tarBallName = getBlobName(BaseActivity.KaryaFileContainer.MICROTASK_ASSIGNMENT_OUTPUT, assignment.id)
    ...
    private suspend fun cleanupKaryaFiles() {
        ...
        for (id in microtaskIds) {
            ...
            val microtaskDirectory = applicationContext.getDir(
