...
@HiltViewModel
class PaymentDetailUPIViewModel @Inject constructor(
    ...
    fun submitUPIDetails(name: String, upiId: String, upiIdRepeated: String) {
        viewModelScope.launch {
            ...
            _uiStateFlow.value = PaymentDetailUPIModel(
                isLoading = true,
                errorMessage = "",
                name = name,
                upiId = upiId,
            ...
            paymentRepository.addAccount(idToken, paymentAccountRequest)
                .catch {
    ...
    private fun validateInputs(
        name: String,
        upiId: String,
        upiIdRepeated: String
    ): Boolean {

        if (name.length < 3) {
            _uiStateFlow.update { it.copy(isLoading = false, errorMessage = "Name cannot be less than 3 characters, please enter your complete name") }
            return false
        }

        if (upiId.isEmpty() || upiIdRepeated.isEmpty()) {
            _uiStateFlow.update { it.copy(isLoading = false, errorMessage = "UPI id cannot be empty, please enter your UPI id") }
            return false
        }
