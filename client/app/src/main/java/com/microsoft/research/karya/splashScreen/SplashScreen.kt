 ...
 */
package com.microsoft.research.karya.splashScreen
...
import com.microsoft.research.karya.fetchData.FirstLoadFetchData
...
class SplashScreen : BaseActivity() {
   ...
   * has not yet chosen any application language. Therefore, it may be wise not to show any text in
  ...
  private fun checkWorkerState() {
    ...
    ioScope.launch {
      ...
      if (workers.isEmpty()) {
        ...
        val appLanguageSkill = workerLanguageSkills.find { it.language_id == worker.app_language }
        ...
        if (appLanguageSkill == null) {
          ...
          nextIntent.putExtra(AppConstants.SKILL_SPECIFICATION_CALLER, AppConstants.SPLASH_SCREEN)
  ...
  private fun updateSplashScreenProgress(state: SplashScreenState) =
      uiScope.launch { splashScreenPb.progress = state.ordinal }
}
