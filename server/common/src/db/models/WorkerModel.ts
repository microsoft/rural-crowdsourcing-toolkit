...
SELECT tw.*, t3.amount FROM 
    (SELECT t2.worker_id, t2.sac-COALESCE(t1.sat,0) as amount FROM (SELECT worker_id, sum(amount)
     ...
     RIGHT JOIN (SELECT worker_id,sum(credits)+sum(max_base_credits) AS SAC FROM microtask_assignment WHERE status IN ('VERIFIED', 'COMPLETED') GROUP BY worker_id) t2
