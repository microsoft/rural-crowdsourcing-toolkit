...
import { baseVideoAnnotationValidation, MicrotaskType } from '@karya/core';
import { BackendChainInterface } from '../BackendChainInterface';

export const videoAnnotationValidation: BackendChainInterface<'VIDEO_ANNOTATION', 'IMAGE_ANNOTATION_VALIDATION'> = {
  ...baseVideoAnnotationValidation,
  ...
  async handleCompletedToMicrotasks(fromTask, toTask, microtasks, assignments) {
    const verificationUpdates = microtasks.map((microtask, i) => {
      ...
      const scores: number[] = ratings.map((r) => (r == 'GOOD' ? 1 : r == 'OKAY' ? 0 : 0));
