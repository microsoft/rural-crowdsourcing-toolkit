...
import { AccountTaskStatus, BulkPaymentsTransactionRecord, BulkTransactionTaskStatus, TransactionRequest } from "@karya/core";
...
export default async (job: Job<BulkTransactionQJobData>) => {
    ...
    {id: bulkTransactionRecord.id}, {status: BulkTransactionTaskStatus.SERVER_BULK_TRANSACTION_QUEUE})
    ...
    for (const transactionRequest of bulkTransactionRequest) {
        try {
            ...
            // Get current account
            const currentActiveAccount = await BasicModel.getSingle('payments_account', { id: workerRecord.selected_account! })
