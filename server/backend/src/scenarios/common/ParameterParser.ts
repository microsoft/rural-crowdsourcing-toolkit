...
export function parseTaskParameters(
  ...
  const defs = scenario.task_params.params;
    ...
    switch (def.type) {
      ...
      case 'integer':
        if (value !== undefined) {
          ...
          if (isNaN(output)) {
            errors.push(
              `Invalid input for parameter '${def.name}'. Was expecting an integer`,
            );
          } else {
      ...
      case 'float':
        if (value !== undefined) {
          ...
          if (isNaN(output)) {
            errors.push(
              `Invalid input for parameter '${def.name}'. Was expecting a number`,
            );
          } else {
            params[identifier] = output;
          }
