...
export async function createWorkProviderTable() {
  await knex.schema.createTable('work_provider', async (table) => {
    ...
    table.specificType('username', 'VARCHAR(48)').unique();
    table.specificType('salt', 'VARCHAR(256)');
    table.specificType('passwd_hash', 'VARCHAR(256)');
    table.specificType('phone_number', 'VARCHAR(16)').unique().notNullable();
    table.specificType('email', 'VARCHAR(256)').unique().notNullable();
    table.specificType('oauth_id', 'VARCHAR(64)');
    table.text('id_token');
    table.json('params').notNullable().defaultTo('{}');
    table
      .timestamp('created_at', { useTz: true })
  ...
  await knex.raw(onUpdateCheckTrigger('work_provider'));
}
